...
if mf_upload:
    try:
        ...
        with colD:
            ...
            # Dynamic variables based on available columns, pulling from the new Micros sheet
            lag_options = [col for col in ['Sodium (mg)', 'Fiber (g)', 'Carbs (g)', 'Calories (kcal)', 'Water (g)'] if col in df.columns]
            
            # Only add Defecation if it has actual data (not just the initialized NAs)
            ...
            if lag_options:
                ...
                # Check if it's a binary categorical (Defecation) or continuous (Micros)
